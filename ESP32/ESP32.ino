...
#endif
      if (Udp.write((byte*)packet, Adhoc.recvBytes) <= 0)
      {
#ifdef __DEBUG__
...
void adhoc_send_filter(char packet[], int numBytes) {
  ...
  */
  ...
  /* Broacast the packet */
  Serial.println("Broadcasting again");
  if (Udp.write((byte*)packet, numBytes) <= 0)
  {
#ifdef __DEBUG__
...
void send_RSSI(char *packet, bool isTCP) {
  ...
  int8_t RSSI_val = Adhoc.get_RSSI();
#ifdef __DEBUG__
  Serial.print("RSSI command received : ");
  Serial.println((int) RSSI_val);
#endif

  char rssi_packet[10];
  memcpy(rssi_packet, packet, 9);
  rssi_packet[7] = 2;
  rssi_packet[9] = RSSI_val;
#ifdef __DEBUG__
  Adhoc.print_packet(rssi_packet, 10);
#endif
  if (isTCP) {
    client.write(rssi_packet, 10);
  } else {
    sendToArduino(rssi_packet, 10);
  }
}
...
void onPacket(const uint8_t* buffer, size_t size) {
  ...
  if (Adhoc.is_internal_cmd(tmp)) {
    ...
    if (Adhoc.is_internal_cmd(tmp) == 2) {
      ...
      // broadcastIP[3] = 255;
      IPAddress broadcastIP = { 224, 0, 250, 1 };
      setLED(BLELED, BLINK);
      if (!Udp.beginMulticast(broadcastIP, UDPport))
      {
#ifdef __DEBUG__
